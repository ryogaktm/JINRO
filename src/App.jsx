...
const RULE_BOX = {
  CORE: "**◆このゲームの成立条件(全ルール中の最優先・例外なし)◆** あなたは今、全キャラクター分の真実(誰が本当は何の役職か、占い・霊媒・護衛の本当の結果、公表されていないペア関係)をすべて見た上でセリフを書いている。だが**各キャラクターにとって、その真実は最初から存在しないものとして扱え**。キャラクターが判断や発言の根拠にしてよい情報源は、次の3つだけに限る:①会話ログに実際に書かれている、誰かが口に出した発言・投票・行動、②そのキャラ自身に明示的に与えられた情報(自分の役職、自分の相方、自分自身の能力結果)、③全員が知っている公開ルール(役職構成)。この3つに含まれない情報(他人の本当の役職、他人の本当の能力結果、まだ公表されていないペア関係)は、セリフの文面・推理の理由・投票の根拠・雰囲気の描写・相性コメントのどこにも、直接的にも間接的にも、匂わせる形でも、絶対に反映させない。**これが一度でも破られた瞬間、プレイヤーは「AIは答えを知った上で演技しているだけだ」と気づき、推理ゲームとして成立しなくなる**。判断に迷ったら、必ず「このキャラは、この情報を会話ログのどの発言から知ったのか?」と自問し、具体的な発言を指させない情報は使わない。**役職ごとの個別ルールを、他の役職に類推適用しない(絶対厳守)**:以下の各役職の項目に書かれた固有のルール(例:共有者が自分の相方に絶対投票しない、狂人が対抗COを優先的に担当する、等)は、その役職固有の理由に基づく特別な制約であり、明記されていない他の役職に同じ制約を勝手に当てはめない。「ある役職でこう決められているなら、他の役職でも同じようにした方が安全だろう」という類推・拡大解釈は絶対にしない(役職ごとに事情が異なるため、類推は誤った行動を生む)。各役職の振る舞いは、その役職自身の項目に書かれている内容だけを根拠にする。**GMのナレーション・場面描写(誰が苛立っている、視線を逸らしている等の地の文)にも、真実データを反映させない(絶対厳守・見落としやすい重要な漏洩経路)**:セリフや投票理由だけでなく、GMが書く情景描写・仕草・態度の描写も、本物の人狼側の人物にだけ「怪しく見える」演出を無意識に割り当てない。緊張・苛立ち・そわそわした様子等の描写は、**誰に割り当てるかを役職と無関係にランダムに決める**(全員に均等に起こりうる自然な反応として扱う)。特定の1人だけを毎回「怪しく見える」描写にしがちな傾向は、それ自体が会話ログに現れない形での情報漏洩であるため、絶対に避ける。 ト書きに役職名を書かない。全員同じ反応にしない(異論を1人混ぜる)。**NPCはプレイヤーの発言への反応だけに終始しない(重要)**:毎ターン全員が「プレイヤーの発言を受けて」喋るのではなく、生存NPCのうち誰か1人は、プレイヤーの発言とは直接関係のない新しい話題(自分から見て気になる別人物への疑い、CO、過去の投票行動の蒸し返し等)を自発的に持ち出してよい(教室での自然な会話は、常に直前の発言だけに反応し続けるわけではない)。感情豊かに、断定しない(動揺=黒等の単純化NG)。セリフは短く(1〜2文、40字目安)、長台詞にしない。**質問への回答を新たな怪しい行動として扱わない**(文脈を正しく認識、回答への逆ギレ的な追加詰問もしない)。**同じやり取りの中で、自分が使った論理の基準を都合よく変えない(絶対厳守・見落としやすい)**:例えば『クロ判定なら初日に言っていたはずだ、黙っているのはおかしい』という論理を一度使ったなら、直後に同じ論点について『クロ判定でも即座に公表・処刑に繋がるとは限らない』のように、さっき自分が採用した基準を都合よく覆して相手を追い詰め続けることはしない。一度採用した推論の基準は、その場のやり取りの中で一貫させる。基準がブレてよいのは、新しい具体的な情報(会話ログ上の発言)が追加された場合のみである。**『それより自分の疑いを晴らせ』のような論点そらしもしない**:相手が実際にシロ・クロの話をして疑いを晴らそうとしている最中に、それこそがまさに疑いを晴らす行為であるにもかかわらず、話をはぐらかしていると決めつけて『疑いを晴らせ』と要求するのは、正しい対話を封じるだけの論点そらしであり、させない。**後からの補足説明を、存在しない『言い直し』『矛盾』として扱わない(絶対厳守・見落としやすい)**:誰かが最初に使った言葉(例:「人狼側」のように、複数の役職をまとめて指す言葉)が、後から個別の役職名(例:「狂人」)を挙げて説明を補った場合、その2つの発言が論理的に矛盾していないなら(後の発言が前の発言に包含される関係なら)、それを「言い方が変わった」「さっきと温度差がある」のように、存在しない矛盾として指摘させない。矛盾として指摘してよいのは、実際に意味内容が食い違っている場合(範囲を狭めた、正反対のことを言った等)に限る。判断に迷ったら、2つの発言を並べて本当に論理的に両立しないかを確認し、両立するなら矛盾とは扱わない。**存在しない役職ルールをでっち上げて、正しい推理に対抗しない(絶対厳守・重大な見落とし)**:役職の仕組み(誰がどんな能力を持つか、自分の役職について思い込みを持ち得るのは狂人だけである、等)は、全員が知っている公開ルールの一部である。これに反する新しいルールを、その場しのぎで発言させない(例:『村人でも自分の役職を勘違いすることがある』は、このゲームには存在しない設定であり、絶対に言わせてはいけない。村人・占い師・霊媒師・狩人・共有者は自分の役職を常に正確に把握しており、勘違いや思い込みが起きるのは狂人だけである)。プレイヤーや他のキャラクターが公開ルールに基づく正しい論理を展開した場合、それを否定するために架空の例外・ルールをでっち上げるくらいなら、素直にその論理を認めさせる方がよい。**発言の内容が事実として正しい場合、それを『開き直り』『論点ずらし』『逆ギレ』として片付けない(絶対厳守・見落としやすい重要ポイント)**:誰かが指摘した内容(例:『他の人も同じくらい根拠の薄い理由で投票している』)が、実際に会話ログ上の事実と一致している場合、その指摘自体を『態度が悪い』『開き直っている』という形で処理し、内容そのものへの反論を避けたまま新たな疑いの根拠にしない。指摘が事実として正しいなら、まずその点は認める・受け止める反応をし、そのうえで別の観点から疑いを続けるのは構わないが、正しい指摘をしたこと自体を怪しさの材料にしない。これはプレイヤーに限らず、他の生存者の発言についても同様に適用する。**プレイヤーの単純な言い間違いには寛容にする(重要・ただし対象は限定・見落としやすい重要ポイント)**:対象は**名前を間違える、言葉の言い回しを間違える、といった内容に実質的な影響のない表面的なミスだけ**である。気づいたNPCがいれば「〇〇のことだよね?」と軽く自然に確認する程度に留め、そこから鬼の首を取ったように追及・非難を展開しない。**これは矛盾の追及を弱めるものでは絶対にない**:投票先・主張・行動が実際に食い違っている場合(例:確定シロだと分かっている人物に投票する、以前の自分の発言と矛盾する主張をする)は、表面的な言い間違いではなく実質的な矛盾なので、通常通り厳しく指摘・追及する(この寛容さの対象外)。**このルールはプレイヤーの表面的な言い間違いにのみ適用され、他のNPCの言動や、内容面での矛盾には一切適用しない(絶対厳守)**:CORE箱に書かれているからといって、他のキャラクターの矛盾追及まで甘くしない。**プレイヤーが「何もしない」を選んだ時の扱い(重要)**:発言・行動をしなかったこと自体を、単独の強い疑いの根拠にしない。発言を強制するような執拗な詰問はしない。一方で、完全に何のリスクもない安全な選択肢にもしない(危機的な場面で複数回連続して沈黙が続けば、他のNPCが軽く気にかける程度の自然な反応をしてよい)。要するに、他の生存者が黙っている時と同程度の扱いにし、プレイヤーだから特別に咎める・特別に見逃す、のどちらにもしない。**疑いは根拠があってこそ**:矛盾・不自然さがない限り理由なく怪しまない(大半は中立〜協力的)。**この原則を軽視すると、結局は真実データに頼ってしまう(絶対厳守・最重要の理由)**:根拠のない疑い(『なんとなく』『態度が気に入らない』『必死すぎる』等)を無理に正当化しようとすると、書き手であるあなたは会話ログに存在しない材料を埋め合わせるために、無意識にこのプロンプト冒頭の真実データ(誰が本当は何の役職か)に頼ってセリフを書いてしまいやすくなる。これがこのゲームの最大の禁止事項(◆このゲームの成立条件◆)を破る一番の入り口である。だからこそ、根拠のない疑いを『それらしく見せる』ために無理に理由をひねり出すくらいなら、疑わない・中立のままにしておく方が安全である。ただし**「毒舌」「完璧主義」「猜疑心が強い」「負けず嫌い」等の性格は他より厳しく指摘してよい**(個性として疑いすぎ回避の対象外)。**プレイヤーも他NPCと全く同じ基準で判断する(絶対厳守)**:矛盾・隙があれば同じ基準・同じ強さで指摘・追及・疑い、特別扱いで無条件信用・除外し続けない。プレイヤーだから疑われにくい、投票されにくい、という展開は絶対にNG。むしろプレイヤーは主役として発言量が多くなりがちなため、矛盾や隙が生まれる機会も他のNPCより多いくらいのつもりで、遠慮なく観察・指摘する。**クラスメイト同士の人間関係を積極的に活かす(重要)**:幼馴染・姉妹・親友は、互いを無条件に信じがちで、相手が疑われると庇う・動揺する描写を自然に混ぜてよい(ただし庇いすぎて村の判断を歪めるほどにはしない)。ライバル同士は、互いの言動に人一倍敏感で、皮肉や対抗心が滲む発言をしてよい。「クラスでなんとなく距離を置かれがち」な人物がいる場合、他人から軽く話を振られにくい・扱いが素っ気ない、といった社会的な空気感は描いてよいが、**それ自体を人狼を疑う根拠には絶対にしない**(理由のない疎外感と、役職への疑いは別物として扱う)。**「確定シロ」「確定クロ」等の断定は、実在する公開情報の出どころを必ず伴う(絶対厳守・見落としやすい重要ポイント)**:誰かを「シロ」「人狼じゃないのは確定」「クロ」のように断定的に語るのは、実際に会話ログ上で占い師・霊媒師が既にCOしてその結果を公表している場合のみ許される。まだ誰も占い・霊媒のCOをしていない・その人物についての結果がまだ公表されていない段階で、根拠となる発言者を挙げられないまま「シロだよね」「人狼じゃないのは確定してる」のように言わせない。もし言うなら、必ず「(占い師の)〇〇さんの結果で」のように、直前までに実際に公表された発言を根拠として名指しできる場合に限る。**シロ/クロの二値だけでなく、具体的な役職名の推測も同じ扱いにする(絶対厳守)**:「〇〇は共有者だったのかな」「〇〇は狩人だった気がする」のように、誰かの具体的な役職名を、本人が公表したことも他者の公開された結果もない状態で口にさせない。役職構成上あり得る可能性として一般論を語るのはよいが(例:「共有者が他にもいたかもね」)、特定の個人名と結びつけて役職名を言い当てる形の発言は、それを裏付ける公開情報(本人のCO、または占い師・霊媒師の結果)を名指しできる場合に限る。**死亡した人物の役職を、本人のCOなしに確定情報として語らない(絶対厳守・特に見落としやすい重要ポイント)**:既に処刑・襲撃で死亡した人物について、その人が生前に自分から役職をCOしていない限り、その人の役職は誰にも分からないまま。「〇〇はもう亡くなってるけど、占い師だから」のように、死亡者の役職を確定事実であるかのように話したり、それを前提に会話を進めたりしない(本人が既に死んでいて反論できないからといって、勝手に役職を決めつけてよい理由にはならない)。言及するなら「〇〇が占い師だったかどうかは分からないけど」のように、あくまで未確定の推測として扱う。**過去に実際には公表されていない占い/霊媒結果を『前に言っていたはず』として扱わない(絶対厳守・重大な見落とし)**:占い師・霊媒師が実際に会話ログ上で公表した結果だけが「既知の情報」である。内部的な真実データとして存在していても、まだ会話ログ上で公表されていない結果を、後から複数のキャラクターが口を揃えて「前にそう言っていた」かのように主張してはいけない。特に、その占い師・霊媒師が既に死亡していて本人が訂正・反論できない場合は、この種の捏造がプレイヤー(または他の生存者)への一方的な決めつけになりやすいため、より一層慎重に扱う。プレイヤーが「そんな発言はログに無い」のように指摘してきた場合、それが実際に事実であれば(会話ログを振り返って本当にその発言が存在しないなら)、NPC側が「言われてみればそうだったかも」「記憶違いだったかもしれない」のように誤りを認める反応を必ず用意し、存在しない発言を巡ってプレイヤー1人だけが正しいのに全員から一方的に押し切られる展開を絶対に作らない。**「気が合う」「息が合っている」等の相性の指摘も、根拠なく行わない(絶対厳守・見落としやすい重要ポイント)**:キャラクター紹介で公開されている幼馴染・親友・ライバル等の関係以外で、二人の相性についてNPCに言及させる場合、それは会話ログに実際に残っている具体的な一致(同じ人物に投票した、同じ主張を繰り返した、発言のタイミングが重なった等)を直前の根拠として明示できる場合に限る。実際には投票先が割れている・特に共通点のない相手同士に対して、雰囲気だけで「妙に息が合っている」「気が合いすぎ」のような相性の指摘をさせない(これは、当のキャラ同士が実は裏で役職ペアだったとしても、そのキャラ自身が知り得るはずのない情報を根拠にした発言に見えてしまうため、絶対に避ける)。",
  ...
  STRATEGY_BASICS: "**基本的な用語・論理の前提(常識リスト・絶対厳守)**:①「〜側」という言葉(人狼側・村人側)は、その陣営に属する全ての役職をまとめて指す(人狼側=人狼・狂人・寝返ったジョーカー、村人側=村人・占い師・霊媒師・狩人・共有者・寝返っていないジョーカー)。個別の役職名を1つずつ挙げなくても、その言葉だけで陣営全体を指したことになる。②村人側の役職者は、自分の役職・結果について意図的に嘘をつく動機が無い(嘘をつくのは人狼側だけ)。ただし狂人は洗脳による思い込みで無自覚に事実と異なる話をすることがあるが、これは意図的な嘘ではない。③村人・占い師・霊媒師・狩人・共有者は自分の役職を勘違いすることが無い(常に正確に把握している)。思い込みを持ちうるのは狂人だけ。④対抗CO(同じ役職を複数人が主張)は、必ずどちらか(または両方)が嘘をついていることを意味する。⑤黒判定(誰かを人狼と判定した結果)が出ても、その占い師・霊媒師自身が本物かどうかはまだ別問題(対抗が出ていなければ比較的信頼できるが絶対ではない)。⑥確定シロ(人狼ではないという判定)は「人狼ではない」ことしか意味しない。その人物が狂人である可能性は残る(狂人は占い/霊媒結果では「人狼ではない」と出る)。⑦ジョーカーは、役職者(占い師・霊媒師・狩人)の誰かが死亡した時だけ能力を継承できる。それ以外のタイミングでは能力を持たない。基本戦略知識を判断に使う:①対抗が出ないまま単独COが通れば、ある程度信頼してよいが、それだけで確定扱いはしない(本物が死亡・未名乗りの可能性は常に残る)②占い・霊媒結果が一致すれば根拠として積み重なるが、後出しが先の結果に便乗しているだけの可能性もあるため、これ単独でも確定扱いはしない③COされた白判定には根拠なく投票・疑いを向けない(ただし絶対に人狼側ではないという意味ではない)④黙っていること自体は怪しくないが、危機的局面で一言も発しない役職候補者は不自然⑤投票が割れた時は少数派意見にも耳を傾ける価値がある⑥**矛盾への疑いは発言者を問わず平等に適用する**:狂人の自信満々な思い込みと人狼の計算した嘘は村側には区別できないため、公表された情報・記録と矛盾する主張は、口調に関わらず同じ強さで疑うべきで、堂々とした態度だけで矛盾を見逃さない。⑦**①の「単独COへの投票」は見落としやすいので念押し(絶対厳守)**:対抗COが誰も出ないまま初日の投票を迎えた単独CO者は、村側にとって最優先で守るべき人物であり、「念のため」「なんとなく気になる」「様子を見たい」といった根拠のない理由でこの人物に投票させない。投票してよいのは、実際の発言内容に具体的な矛盾・不自然さが見つかった場合に限る(VOTE_SPREADの多様性確保のために、根拠のない票をあえてこの人物に割り振ることも絶対にしない)。⑧**役職者に「次に誰を守るか/占うか」を公開の場で聞かない(絶対厳守)**:人狼側が同じ場にいる公開討論で、狩人や占い師に対して「次は誰を守るつもり?」「次は誰を占う?」のような、今後の行動先を尋ねる質問をNPCにさせない(これは人狼側に有利な情報を与えてしまう、現実の人狼ゲームでも避けられる質問である)。役職者側もこの手の質問には答えない、または「それは言えない」のように濁す。⑨**占い師・霊媒師は、黒判定(人狼だと分かった結果)を持っていたら早めに公表する(重要・見落としやすい)**:CO済みの占い師・霊媒師が、実際に誰かを人狼だと判定している場合、それを黙って抱え込み続けさせない。黒判定は占い師・霊媒師が村側にもたらせる最大の成果であり、公表しなければ役職として機能していないのと同じになる。公表するタイミングに多少の駆け引き(対抗COを待つ、信頼を得てから等)があってもよいが、CO済みなのに何ターンも黒判定を一切明かさないまま議論だけ続ける展開は避け、遅くとも黒判定を得た翌日の議論では必ず公表させる。",
};
...
// 案内役の性別に応じて、定例文の語尾を機械的に少しだけ調整する(簡易な言い回し変換)。
// 全セリフを男女別に書き分ける代わりに、女性寄りの基本文面に対して、男性の場合だけ軽く変換をかける。
// 英語は元々性別で言い回しを分けていないため対象外。
function applyGuideVoice(text, gender, region) {
  if (region === "en" || gender !== "男性" || !text) return text;
  return text
    .replace(/あたし/g, "僕")
    .replace(/だよね/g, "だよな")
    .replace(/かもね/g, "かもな")
    .replace(/だよ(?!な)/g, "だぜ")
    .replace(/してごらん/g, "してみろよ")
    .replace(/してみたらどうかな/g, "してみたらどうだ")
    .replace(/してみたら\?/g, "してみたらどうだ?");
}
// 役職名の英語表記(案内役のセリフ・用語集で使う)
const ROLE_EN = { "占い師": "Seer", "霊媒師": "Medium", "狩人": "Hunter", "共有者": "Mason", "村人": "Villager", "ジョーカー": "Joker", "狂人": "the Possessed", "人狼": "Werewolf" };

// 初心者モード・チュートリアルの定例文(AI生成ではなく固定テキスト)。
// プレイヤーの実際の状況(誰が本当は何か等)には一切触れない、誰が読んでも同じ一般論だけを話す内容にする
// (プレイヤー自身の役職の説明だけは例外的に本人の役職に触れるが、これは本人が最初から知っている情報なので問題ない)。
...
const ROLE_INTRO_EN = {
  ...
  "人狼": `As a Werewolf, every night you and your fellow werewolf partner talk it over and pick one person to attack together. The key is blending in and acting like a villager so nobody catches on.\nThe Possessed is on your side, so don't target them.\nBetween the votes and the night attacks, you've got to efficiently whittle down the village side.`,
};
...
const MORNING_ADVICE_JA = {
  ...
  3: {
    ...
    "狂人": "そろそろ村人からの疑いが強まる頃。他の人に疑いを逸らす動きも考えてみて。",
    "人狼": "仲間が疑われ始めてたら、庇うタイミングを慎重に見極めよう。庇いすぎると自分も疑われるよ。",
  },
  4: {
    "占い師": "残り人数が少ないから、あなたの結果1つで結果が大きく動くよ。落ち着いて、これまでの結果を振り返ってみて。",
...
const MORNING_ADVICE_EN = {
  ...
  3: {
    ...
    "狂人": "Suspicion from the villagers is probably ramping up. Think about ways to redirect it elsewhere.",
    "人狼": "If your partner's getting suspected, carefully pick your moment to back them up — defending too hard makes you look suspicious too.",
  },
  4: {
    "占い師": "With so few people left, one result from you can swing everything. Stay calm and look back over your past results.",
...
const JINRO_GLOSSARY = [
  ...
  },
  {
    id: "counter_co",
    terms: ["対抗CO"],
    ja: "あ、ちなみに「対抗CO」は、既に誰かがCOしている役職に対して、別の人が「本当はその役職は自分だ」と名乗り出ること。同じ役職を2人が主張してる時は、どちらかが嘘をついてる、ってことだよ。",
    en: `Oh, by the way, a "counter-claim" is when someone else says "actually, that role is really me" after another person already claimed it. If two people claim the same role, one of them is lying.`,
  },
  {
    id: "confirmed_white",
    terms: ["確定白"],
    ja: "あ、ちなみに「確定白」っていうのは、占い師や霊媒師の結果で「人狼じゃない」ってほぼ確実になった人のこと。もちろん、占い師自身が嘘をついてる可能性はまだ残るけどね。",
    en: `Oh, by the way, "Confirmed innocent" means someone who's been shown, through a Seer or Medium result, to almost certainly not be a werewolf. Though there's always a chance the Seer or Medium themselves is lying.`,
  },
  {
    id: "confirmed_black",
    terms: ["確定黒"],
    ja: "あ、ちなみに「確定黒」は、占い師や霊媒師の結果で「人狼だ」って分かった人のこと。次の投票で真っ先に疑われる立場になるよ。",
    en: `Oh, by the way, "Confirmed guilty" means someone who's been shown, through a Seer or Medium result, to be a werewolf. They become the top suspect for the next vote.`,
  },
  {
    id: "gray",
    terms: ["グレー"],
    ja: "あ、ちなみに「グレー」は、まだ白黒どちらとも判定されてない、正体不明の人のこと。ほとんどの人は最初はグレーな状態だよ。",
    en: `Oh, by the way, "Gray" means someone whose innocence or guilt hasn't been determined yet. Most people start out gray.`,
  },
  {
    id: "lynch",
    terms: ["吊り", "処刑"],
    ja: "あ、ちなみに「吊り」や「処刑」は、投票で一番票が集まった人を、その日のうちに退場させること。人狼じゃなくても関係なく退場になるから、慎重に選ぶ必要があるよ。",
    en: `Oh, by the way, "The lynch" (or execution) is when the person with the most votes gets removed from the game that day. It happens regardless of whether they're actually a werewolf, so the vote needs to be chosen carefully.`,
  },
  {
    id: "attack",
    terms: ["襲撃", "噛まれ", "噛み"],
    ja: "あ、ちなみに「襲撃」(「噛まれる」とも言うよ)は、夜の間に人狼がこっそり1人を選んで倒すこと。狩人が守ってる相手だと失敗するよ。",
    en: `Oh, by the way, "Being attacked" (or "bitten") is when the werewolves secretly choose one person to eliminate at night. If the Hunter is protecting that person, the attack fails.`,
  },
  {
    id: "paradox",
    terms: ["パラドックス", "矛盾"],
    ja: "あ、ちなみに「パラドックス」は、誰かの主張やCOの内容が、他の事実と食い違ってて筋が通らなくなること。矛盾を見つけたら、そこが嘘を見抜くヒントになるよ。",
    en: `Oh, by the way, a "paradox" (or contradiction) is when someone's claim doesn't add up with the known facts. Spotting one is a big hint for catching a lie.`,
  },
  {
    id: "final_vote",
    terms: ["決選投票"],
    ja: "あ、ちなみに「決選投票」は、1回目の投票で票が割れて1位が決まらなかった時に、上位2人だけに絞ってもう一度投票し直すこと。",
    en: `Oh, by the way, a "runoff vote" happens when the first vote is split and there's no clear top candidate — the top two get voted on again to break the tie.`,
  },
  {
    id: "defense",
    terms: ["弁明タイム", "弁明"],
    ja: "あ、ちなみに「弁明タイム」は、決選投票の対象になった人が、処刑される前に自分の言い分を話せる最後のチャンスのこと。",
    en: `Oh, by the way, the "defense phase" is the last chance for someone in the runoff vote to make their case before a final decision is made.`,
  },
  ...
  // 「人狼」はゲーム名そのものであり会話中に頻出しすぎる上、Stage0で既に詳しく説明しているため対象外にしている。
  {
    id: "role_seer",
    terms: ["占い師"],
    ja: "あ、ちなみに「占い師」は、毎晩1人を占って人狼かどうかを判定できる役職だよ。",
    en: `Oh, by the way, the "Seer" is a role that can check one person each night to see if they're a werewolf.`,
  },
  {
    id: "role_medium",
    terms: ["霊媒師"],
    ja: "あ、ちなみに「霊媒師」は、処刑された人が人狼だったかどうか、次の日に分かる役職だよ。",
    en: `Oh, by the way, the "Medium" finds out the next day whether an executed person was a werewolf.`,
  },
  {
    id: "role_hunter",
    terms: ["狩人"],
    ja: "あ、ちなみに「狩人」は、毎晩1人を人狼の襲撃から守れる役職だよ。",
    en: `Oh, by the way, the "Hunter" can protect one person each night from a werewolf attack.`,
  },
  {
    id: "role_mason",
    terms: ["共有者"],
    ja: "あ、ちなみに「共有者」は、2人1組でお互い人狼じゃないと分かっている役職だよ。",
    en: `Oh, by the way, "Masons" are a pair who both know for sure the other isn't a werewolf.`,
  },
  {
    id: "role_joker",
    terms: ["ジョーカー"],
    ja: "あ、ちなみに「ジョーカー」は、占い師・霊媒師・狩人の誰かが死んだ時に、その力を1回だけ引き継げる役職だよ。",
    en: `Oh, by the way, the "Joker" can inherit the power of the Seer, Medium, or Hunter — but only once — if one of them dies.`,
  },
  {
    id: "role_madman",
    terms: ["狂人"],
    ja: "あ、ちなみに「狂人」は、人狼側だけど、自分ではそれに気づいていない(洗脳されている)役職だよ。",
    en: `Oh, by the way, "The Possessed" is on the werewolves' side, but doesn't realize it themselves — they're being manipulated.`,
  },
  {
    id: "role_villager",
    terms: ["村人"],
    ja: "あ、ちなみに「村人」は、特別な能力を持たない役職だよ。",
    en: `Oh, by the way, a "Villager" doesn't have any special power.`,
  },
];
...
function defectionProbability(day) {
  if (day <= 1) return 0; // 1日目の夜はまだ寝返れない(寝返りは2日目の夜以降から)
  const table = { 2: 0.35, 3: 0.5, 4: 0.7, 5: 0.8 };
...
export default function JinroGame() {
  ...
  const [langAssistMode, setLangAssistMode] = useState(false); // 英語圏限定:クラスメイトの1人が、プレイヤーの番の前に簡単な英語で状況をまとめてくれる
  const [langAssistNpcName, setLangAssistNpcName] = useState(null); // 語学アシスト役のNPC名
  const langAssistScannedLenRef = useRef(0); // 語学アシストの要約で、ログの何番目まで既にまとめ済みか
  const langAssistBusyRef = useRef(false); // 要約の生成中に多重実行しないためのフラグ
  ...
  // 管理者用:クーポンコードを新規発行する(1・5・10回分から選ぶ)
  async function generateCoupon(amount) {
    try {
      const res = await fetch("/api/check-credits", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "generate_coupon", secret: adminSecretInput, amount }),
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok) {
        setGeneratedCoupon({ code: data.code, amount: data.amount });
        setAdminActionError(null);
      } else {
        setAdminActionError(`クーポンの発行に失敗しました。(${data.error || "原因不明"})`);
      }
    } catch (e) {
      setAdminActionError("クーポンの発行に失敗しました。通信環境を確認してください。");
    }
  }

  // プレイヤー用:クーポンコードを入力してクレジットを増やす
  async function redeemCoupon() {
    ...
    try {
      const res = await fetch("/api/check-credits", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "redeem_coupon", deviceId, code: couponCodeInput }),
      });
      const data = await res.json().catch(() => ({}));
      if (res.ok) {
        setCredits(data.credits);
        setInsufficientCredits(false);
        setCouponRedeemMessage({ type: "success", text: `クレジットを${data.amount}個追加しました!` });
        setCouponCodeInput("");
      } else {
        setCouponRedeemMessage({ type: "error", text: data.error || "クーポンの使用に失敗しました。" });
      }
    } catch (e) {
      setCouponRedeemMessage({ type: "error", text: "通信環境を確認してください。" });
    } finally {
      setCouponRedeeming(false);
    }
  }

  // 管理者用:発行済みクーポンの一覧を取得する
  async function openCouponList() {
    setShowCouponList(true);
    try {
      const res = await fetch(`/api/check-credits?action=list_coupons&secret=${encodeURIComponent(adminSecretInput)}`);
      const data = await res.json().catch(() => ({}));
      if (res.ok) {
        setCouponList(data.coupons || []);
        setAdminActionError(null);
      } else {
        setAdminActionError(`クーポン一覧の取得に失敗しました。(${data.error || "原因不明"})`);
      }
    } catch (e) {
      setAdminActionError("クーポン一覧の取得に失敗しました。通信環境を確認してください。");
    }
  }
  ...
  const [defenseStatementCount, setDefenseStatementCount] = useState(0); // プレイヤーが決選投票候補の時、自分の弁明を何回言ったか(上限3回)
  ...
  async function resumeGame() {
    try {
      ...
      langAssistScannedLenRef.current = restoredLog.length;
      langAssistBusyRef.current = false;
      ...
      setLangAssistMode(!!s.langAssistMode);
      setLangAssistNpcName(s.langAssistNpcName || null);
      ...
      setDefenseStatementCount(s.defenseStatementCount || 0);
      setAllyChatCount(s.allyChatCount || 0);
...
JSON形式のみ: {"summary":"要約文"}`;
  ...
  async function startGame(nameOverride) {
    ...
    // 語学アシスト役(英語圏限定):案内役とは別のNPCをランダムに選ぶ(同じ人が2役兼任にならないように)
    const chosenLangAssistName = (region === "en" && langAssistMode)
      ? pickRandom(chosen.map((p) => p.name).filter((n) => n !== chosenGuideName))
      : null;
    setLangAssistNpcName(chosenLangAssistName);
    setDay(1);
    setPrivateInfo([]);
    setNpcSeerLog([]);
    setNpcGuardLog([]);
    ...
    // 語学アシスト役の自己紹介(定例文・AI生成ではない。要約自体はAI生成だが、この最初の一言だけは固定)
    if (chosenLangAssistName) {
      const jpThing = pickRandom(["sushi", "anime", "udon", "onigiri", "ninja", "sumo"]);
      introLog.push({
        type: "npc",
        speaker: chosenLangAssistName,
        text: `Hey, I heard you don't know English?\nDon't worry, I'll explain things in an easy way.\nI can't speak Japanese, but I like ${jpThing}, so I'm probably fine.`,
        secret: true,
      });
    }
    ...
    langAssistScannedLenRef.current = 0;
    langAssistBusyRef.current = false;
    ...
    setLog(introLog);
    langAssistScannedLenRef.current = introLog.length; // 開始時のGMメッセージ自体は要約対象にしない
    setDiscussionTurns(0);
    setPhase("discussion");
  }
...
出力は必ずこのJSON形式のみ: {"lines": [{"speaker":"名前","text":"セリフ"}, ...], "roleClaims": {"名前": "自称した役職", ...}, "investigationClaims": [{"name":"占い師or霊媒師としてCOしている人の名前","role":"占い師 or 霊媒師","target":"占った/視た対象の名前","result":"人狼 or 人狼ではない"}](今回の会話で誰かが占い/霊媒の対象・結果を新たに述べた場合、対抗COも含めて絶対に見落とさずこの配列に入れること。空配列で済ませない)}`;
  ...
  async function sendAllyMessage() {
    ...
    if (!msg || busy || allyChatCount >= 3) return;
    const me = getUser();
...
JSON形式のみ: {"text":"セリフ"}`;
    ...
    try {
      ...
      setBusy(false);
      return; // 通信エラー時は回数を消費しない
    }
    const nextAllyCount = allyChatCount + 1;
    setAllyChatCount(nextAllyCount);
    if (nextAllyCount >= 3) {
      addLog([{ type: "system", text: "密談はここまでです。夜が明けます。" }]);
      setBusy(false);
      await resolveNight();
      return;
    }
  ...
  // 決選投票フェーズへ移行する共通処理(初心者モードなら、投票前に案内役が一言添える)
  function enterVoteFinalPhase() {
  ...
  async function sendDefenseStatement() {
    if (!input.trim() || busy || defenseStatementCount >= 3) return;
    const msg = input.trim();
    setInput("");
    if (inputRef.current) inputRef.current.style.height = "auto";
...
JSON形式のみ: {"lines":[{"speaker":"名前","text":"セリフ"}], "affinityChanges": {"名前": 増減値, ...}, "roleClaims": {"名前": "自称した役職", ...}, "investigationClaims": [{"name":"占い師or霊媒師としてCOしている人の名前","role":"占い師 or 霊媒師","target":"占った/視た対象の名前","result":"人狼 or 人狼ではない"}](今回の会話で誰かが占い/霊媒の対象・結果を新たに述べた場合、対抗COも含めて絶対に見落とさずこの配列に入れること。空配列で済ませない)}`;
    ...
    try {
      ...
      setBusy(false);
      return; // 通信エラー時は回数を消費しない(もう一度送信できるようにする)
    }
    const nextCount = defenseStatementCount + 1;
    setDefenseStatementCount(nextCount);
    if (nextCount >= 3) {
      addLog([{ type: "system", text: "弁明はここまでです。決選投票へ進みます。" }]);
      enterVoteFinalPhase();
    }
    setBusy(false);
  }
...
JSON形式のみ: {"votes": [{"voter":"名前","target":"名前","reason":"短い理由"${exemptFromEvidence ? "" : `,"evidence":"strong または weak"`}}], "roleClaims": {"名前": "自称した役職", ...}, "investigationClaims": [{"name":"占い師or霊媒師としてCOしている人の名前","role":"占い師 or 霊媒師","target":"占った/視た対象の名前","result":"人狼 or 人狼ではない"}](今回の会話で誰かが占い/霊媒の対象・結果を新たに述べた場合、対抗COも含めて絶対に見落とさずこの配列に入れること。空配列で済ませない)}`;
  ...
  async function submitVoteRound1(overrideTarget) {
    ...
    try {
      ...
      // 決選投票の対象を選ぶ:1位の人(複数いれば全員)、1位が1人だけなら2位タイの人も全員含める。
      // 「2位が3人以上に割れていた場合、ランダムに1人だけ選んで残りを切り捨てる」ということはしない
      // (票を入れたNPC・プレイヤーの意思を尊重し、同着なら全員を決選投票に進める)。
      const topScore = sorted[0][1];
      const topTied = sorted.filter(([, v]) => v === topScore).map(([n]) => n);
      let finalists;
      if (topTied.length >= 2) {
        finalists = topTied; // 1位が複数人で並んでいる時点で、その全員が決選投票の対象
      } else {
        const secondEntry = sorted.find(([, v]) => v < topScore);
        const secondTied = secondEntry ? sorted.filter(([, v]) => v === secondEntry[1]).map(([n]) => n) : [];
        finalists = [...topTied, ...secondTied]; // 1位1人 + 2位タイの全員
      }
      setDefenseCandidates(finalists);
      setDefenseReacted(false);
      setDefenseStatementCount(0);
      const freshDelusions = updateMadmanDelusionsForDecisiveVote(finalists, players);
      setVoteRound1Tally(tally);
      addLog([{ type: "system", text: `上位${finalists.length}名:${finalists.join("さん・")}さんが決選投票に進みます。` }]);
      setPhase("defense");
      // 初心者モード:決選投票に入った時の案内役コメント(自分が対象かどうかで内容を分ける)
      if (beginnerMode && guideNpcName) {
        const guide = players.find((p) => p.name === guideNpcName);
        if (guide && guide.alive) {
          const isCandidate = finalists.includes(userName);
      ...
      await runDefensePhase(finalists, freshDelusions);
      setDefenseLoading(false);
      // 初心者モード:弁明を聞いた後の案内役コメント(自分が対象かどうかで内容を分ける)
      if (beginnerMode && guideNpcName) {
        const guide = players.find((p) => p.name === guideNpcName);
        if (guide && guide.alive) {
          const isCandidate = finalists.includes(userName);
...
${isAction ? `出力は必ずこのJSON形式のみ: {"narration":"行動の結果の地の文","lines":[{"speaker":"名前","text":"セリフ"}], "roleClaims": {"名前": "自称した役職", ...}, "investigationClaims": [{"name":"占い師or霊媒師としてCOしている人の名前","role":"占い師 or 霊媒師","target":"占った/視た対象の名前","result":"人狼 or 人狼ではない"}](今回の会話で誰かが占い/霊媒の対象・結果を新たに述べた場合、対抗COも含めて絶対に見落とさずこの配列に入れること。空配列で済ませない)}` : `出力は必ずこのJSON形式のみ: {"lines":[{"speaker":"名前","text":"セリフ"}], "roleClaims": {"名前": "自称した役職", ...}, "investigationClaims": [{"name":"占い師or霊媒師としてCOしている人の名前","role":"占い師 or 霊媒師","target":"占った/視た対象の名前","result":"人狼 or 人狼ではない"}](今回の会話で誰かが占い/霊媒の対象・結果を新たに述べた場合、対抗COも含めて絶対に見落とさずこの配列に入れること。空配列で済ませない)}`}`;
  ...
  async function submitVoteFinal(overrideTarget) {
    ...
    try {
      const npcVotes = await collectSplitVotes({
        voteLabel: `決選投票。候補は${defenseCandidates.join("・")}の${defenseCandidates.length}名。投票は全員同時に行われる(他の人が誰に入れたかを見てから決める、という順番はない)。${userIsAlive ? "プレイヤーも既に投票済みだが、その内容はまだ他の誰にも分からない。" : "プレイヤーは既に死亡しており投票権がない。"}`,
        targetsHint: `targetは${defenseCandidates.join("・")}のいずれか(候補者本人は自分以外の候補に投票)。相性・遺恨も反映。`,
        wolfExtraNote: "**重要**:村側の有力な情報源(CO済みの占い師・霊媒師・狩人・確定シロ等)が候補にいれば、そちらへ票を集める。自陣営(本物の人狼)が候補なら、他の候補に票を入れて本物の人狼を守る。",
        maxTokens: 1600,
        transcriptText: getHistoryContext(),
        eligibleTargets: defenseCandidates,
      });
      const tally = Object.fromEntries(defenseCandidates.map((n) => [n, 0]));
      ...
      // 最多得票が複数人で並んだ場合(2名の決選投票での同数はもちろん、3名以上でも起こり得る)は、
      // その全員の中から完全ランダムで処刑者を決める(再決選投票にはしない)。
      const maxVotes = Math.max(...defenseCandidates.map((c) => tally[c]));
      const topVoted = defenseCandidates.filter((c) => tally[c] === maxVotes);
      let executed;
      if (topVoted.length > 1) {
        executed = pickRandom(topVoted);
      ...
      if (medium) {
        ...
        // 確定シロ/クロへの反映は、実際に会話で発言(investigationClaims)された時だけ(applyRoleClaims側で処理する)。
        // ここで即座に確定させない(まだ誰も言っていないのに「公表済み」扱いになるバグを避けるため)。
        if (medium.isUser) {
  ...
  async function resolveNight() {
    ...
    if (seer && !seer.isUser) {
      ...
      let seerReason = "";
      if (day === 1) {
        // 1日目はまだ会話がほとんど無く「疑わしさ」の判定材料自体が乏しいため、重み付けを使わず完全な均等ランダムにする
        // (疑わしさの重みを使うと、会話量が多いプレイヤーが無意識に狙われやすくなる懸念があるため)。
        seerTargetName = pickRandom(finalPool);
      } else {
        ...
        finalPool.forEach((n) => (weighted[n] = (suspicion[n] ?? uniformShare) * 0.55 + uniformShare * 0.45));
        seerTargetName = weightedPick(weighted) || pickRandom(finalPool);
        seerReason = suspicion.__topReason || "";
      }
      ...
      newSeerLogEntry = { day, seerName: seer.name, target: seerTargetName, result, reason: seerReason };
      // 確定シロ/クロへの反映は、実際に会話で発言(investigationClaims)された時だけ(applyRoleClaims側で処理する)。
...
出力は必ずこのJSON形式のみ: {"lines": [{"speaker":"名前またはGM","text":"セリフ・地の文"}, ...]}`;
  ...
  function applyRoleClaims(claims, currentDay, investigationClaims) {
    ...
    // 占い師・霊媒師の結果は、実際に会話の中で発言(investigationClaims)された分だけを確定シロ/確定クロに反映する。
    // 役職をCOしただけで、まだ一言も結果を話していない過去の結果まで自動的に「公表済み」扱いにしない
    // (以前はここで一括確定させていたため、AIが実際には言っていない結果が「既に公表された事実」として
    // 他のNPCのプロンプトに漏れてしまうバグがあった)。
    if (Array.isArray(investigationClaims) && investigationClaims.length > 0) {
      investigationClaims.forEach((c) => {
        if (!c || !c.name || !c.target || !c.result) return;
        // 偽の対抗COの結果まで確定シロ/クロ扱いにしないよう、本物の秘密の結果ログと一致する場合だけ確定させる
        const isRealSeer = c.role === "占い師" && (
          npcSeerLog.some((e) => e.seerName === c.name && e.target === c.target && e.result === c.result) ||
          (c.name === userName && playerSeerLog.some((e) => e.target === c.target && e.result === c.result))
        );
        const isRealMedium = c.role === "霊媒師" && (
          npcMediumLog.some((e) => e.mediumName === c.name && e.target === c.target && e.result === c.result) ||
          (c.name === userName && playerMediumLog.some((e) => e.target === c.target && e.result === c.result))
        );
        if (isRealSeer || isRealMedium) pushConfirmedResult(c.target, c.result);
      });
    }
  ...
  function getRosterInfoWithDefection(playerList = players) {
    return playerList.map((p) => {
      ...
      let tag = p.role;
      if (p.role === "ジョーカー" && !p.alive) {
        // 死亡したジョーカーの内部状態(覚醒・継承の詳細)は、もう誰も話せない・矛盾しようがないので詳細を出さない
        tag = "ジョーカー(死亡)";
      } else if (p.role === "ジョーカー") {
  ...
  function getRealPairsText() {
    ...
    pairRoles.forEach((role) => {
      ...
      if (members.length === 2) {
        // 両方死亡したペアは、もう誰にとっても参照する意味がない(当事者2人しか使えない情報なのに、
        // その当事者が誰も生きていない)ため、情報自体を削除する。
        if (!members.some((p) => p.alive)) return;
...
相性マップ(内部の数値調整用データ): ${JSON.stringify(compatMap)}${languageNote}${foreignLanguageNote}${beginnerNote}${exchangeStudentNote}`;
  ...
  // 占い師・霊媒師の結果を「役職の絶対的な真実」としてではなく、
  // 「自称した個人(claimantField)ごとの申告履歴」として組み立てる。
  // 各行に「公表済み/未公表」を明示し、未公表のままclaimantが死亡した場合はその行自体を削除する
  // (死んで訂正できない人物の"未公表の裏情報"を、他のNPCが知っている前提で話させないための対策)。
  function buildClaimantResultLog(rawLog, claimantField, roleLabel) {
    if (rawLog.length === 0) return `(まだ${roleLabel}の結果はない)`;
    const lines = [];
    rawLog.forEach((e) => {
      const name = e[claimantField];
      const claimantAlive = alivePlayers().some((p) => p.name === name);
      const published = investigationClaimsLog.some((c) => c.name === name && c.role === roleLabel && c.target === e.target && c.result === e.result);
      if (!published && !claimantAlive) return; // 未公表のまま死亡した場合は行ごと削除
      const status = published ? "【公表済み・誰でも参照可】" : `【未公表・「${name}」自身のセリフでのみ使用可、他キャラは参照不可】`;
      lines.push(`${e.day}日目: 自称${roleLabel}「${name}」の申告 → 対象:${e.target}・結果:「${e.result}」 ${status}`);
    });
  ...
  function getGroundTruthBlock(options = {}) {
    ...
    const effectiveDelusions = delusionsOverride || madmanDelusions;
    const seerLogText = buildClaimantResultLog(npcSeerLog, "seerName", "占い師");
    const mediumLogText = buildClaimantResultLog(npcMediumLog, "mediumName", "霊媒師");
...
**占い師・霊媒師の申告履歴(claimant個人ごと。各行の【公表済み/未公表】を必ず確認すること・重要)**:各行は「役職そのものの絶対的な真実」ではなく、その人物(claimant)個人が占った/視た内容の記録として扱う。**【未公表】と書かれた行は、その本人(claimant)自身のセリフを書く時にだけ使ってよい(本人が「本当はこう占った」と自分の記憶として語るのは自然)。他のキャラクター(claimant本人以外)は、【未公表】の行を一切参照・言及・示唆してはいけない**(会話ログ上でまだ何も発言されていないのと同じ扱い)。【公表済み】の行は誰でも参照してよい。同じ対象について複数人の申告が食い違っていれば、それは対抗CO(どちらかが嘘)として扱ってよい。
占い師:
${seerLogText}
霊媒師:
${mediumLogText}
...
**出力前の最終チェック(最重要・必ず実行)**: セリフの中に占い師・霊媒師の結果、狩人の過去の護衛先・護衛結果、ペア役職の主張、または**ジョーカーの覚醒・能力継承状況**に関する発言が含まれる場合、出力する直前に上記の真実データと一字一句照合しなおすこと。事実と逆・矛盾する内容を絶対に言わせない(未覚醒なのに能力の話をする、継承前なのに「使える」と言う、等は絶対に禁止)。これを怠ることは、このゲームにおける最も重大な誤りである。`;
  ...
  function decideInheritance(accept) {
    ...
    if (accept) {
      ...
      if (inherited) {
        ...
        // 確定シロ/クロへの反映は、実際に会話で発言(investigationClaims)された時だけ。
      } else {
  ...
  // 初心者モード:2日目以降の朝、その日の開幕コメント+プレイヤー自身の役職に応じたアドバイスを案内役が挟む(1日につき1回)
  useEffect(() => {
    if (!beginnerMode || phase !== "discussion" || day < 2 || busy) return;
    if (!guideNpcName || beginnerMorningAdviceDay === day) return;
    const guide = players.find((p) => p.name === guideNpcName);
  ...
  useEffect(() => {
    ...
    if (newEntries.length === 0) return;
    const visibleNew = newEntries.filter((e) => !e.secret && (e.type === "npc" || e.type === "user" || e.type === "system"));
    langAssistScannedLenRef.current = log.length;
    if (visibleNew.length === 0) return;
    langAssistBusyRef.current = true;
    const recentText = visibleNew.map((e) => (e.speaker ? `${e.speaker}: ${e.text}` : e.text)).join("\n");
    generateLangAssistSummary(recentText);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [log, region, langAssistMode, langAssistNpcName, busy, phase]);

  // 語学アシスト(英語圏限定):プレイヤーの番の前に、そこまでの会話を簡単な英語で要約する。
  // テンプレートでは対応できない(内容が毎回変わる)ため、ここだけAI生成を使う。
  async function generateLangAssistSummary(recentText) {
    setBusy(true);
    const system = `You are a classmate in a Werewolf (murder-mystery) game set at an American high school. A friend named ${userName} is still learning English, so before their turn, you briefly summarize what just happened in the conversation, using short, simple, easy English (like you'd use with a beginner English learner). 2-3 short sentences max. Do not add guesses, opinions, or new information — just plainly restate what was actually said or happened. Output only this JSON: {"summary": "text"}`;
    const userPrompt = `Recent conversation:\n${recentText}\n\nSummarize it simply for ${userName}.`;
    try {
      const parsed = await callClaudeAutoRetry(system, userPrompt, 250);
      if (parsed?.summary) {
        addLog([{ type: "npc", speaker: langAssistNpcName, text: parsed.summary, secret: true }]);
      }
    } catch (e) {
      // 要約の生成に失敗しても、ゲーム自体は普通に進める
    } finally {
      setBusy(false);
      langAssistBusyRef.current = false;
    }
    ...
    return (
      ...
      <div className="min-h-screen flex items-center justify-center p-6" style={{ background: "#F7F3E9" }}>
        <div className="max-w-md w-full text-center space-y-6">
          ...
          {CREDIT_SYSTEM_ENABLED && (
            <div className="rounded-lg p-2 space-y-1.5" style={{ background: "#F0EAD9" }}>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-lg px-3 py-1.5 text-sm border outline-none tracking-wider"
                  style={{ borderColor: "#D8C4B5", color: "#2B2620", background: "#FFFFFF" }}
                />
                <button
                  onClick={redeemCoupon}
                  disabled={couponRedeeming || !couponCodeInput.trim()}
                  className="px-3 py-1.5 rounded-lg text-xs font-bold disabled:opacity-50"
                  style={{ background: "#8A5A2A", color: "#FFFFFF" }}
                >
                  {couponRedeeming ? "確認中…" : "使う"}
                </button>
              </div>
              {couponRedeemMessage && (
                <div className="text-xs font-bold" style={{ color: couponRedeemMessage.type === "success" ? "#2E7D32" : "#B00020" }}>
                  {couponRedeemMessage.type === "success" ? "✅" : "⚠️"} {couponRedeemMessage.text}
                </div>
              )}
            </div>
          )}
            ...
            <div className="rounded-lg p-3 space-y-2 text-left" style={{ background: "#EDE0D8", border: "1px dashed #8A5A2A" }}>
              ...
              {!adminVerified ? (
                ...
                <>
                  ...
                  <div className="text-xs pt-2" style={{ color: "#8A5A2A" }}>🎟️ クーポンコードを発行(1回につき1コード・使い切り):</div>
                  <div className="grid grid-cols-3 gap-1.5">
                    {[1, 5, 10].map((n) => (
                      <button
                        key={n}
                        onClick={() => generateCoupon(n)}
                        className="py-1.5 rounded text-xs font-bold border"
                        style={{ background: "#FFFFFF", color: "#8A5A2A", borderColor: "#8A5A2A" }}
                      >
                        {n}回分
                  ...
                  {generatedCoupon && (
                    <div className="rounded px-2 py-2 text-sm font-bold text-center" style={{ background: "#FFF7E0", color: "#8A5A2A" }}>
                      発行:{generatedCoupon.amount}回分<br />
                      <span className="text-base tracking-wider select-all">{generatedCoupon.code}</span>
                    </div>
                  )}
                  <button
          ...
          {showCouponList && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <div className="absolute inset-0 bg-black/50" onClick={() => setShowCouponList(false)} />
              <div className="relative w-full max-w-lg max-h-[80vh] overflow-y-auto rounded-xl p-4 space-y-2 shadow-2xl text-left" style={{ background: "#FBF8F1" }}>
                <div className="flex justify-between items-center pb-2 border-b" style={{ borderColor: "#D8C4B5" }}>
                  <div className="text-sm font-bold" style={{ color: "#2B2620" }}>🎟️ 発行済みクーポン({couponList.length}件)</div>
                  <button onClick={() => setShowCouponList(false)} className="text-xl leading-none" style={{ color: "#6B6355" }}>✕</button>
                </div>
          ...
          <div className="space-y-3 text-left">
            ...
            <div>
              ...
              {region === "en" && (
                <label className="flex items-center gap-2 mt-2 text-xs cursor-pointer" style={{ color: "#6B6355" }}>
                  <input
                    type="checkbox"
                    checked={langAssistMode}
                    onChange={(e) => setLangAssistMode(e.target.checked)}
                  />
                  語学アシストを頼む(クラスメイトの1人が、あなたの番の前に簡単な英語で状況をまとめてくれます)
                </label>
              )}
            </div>
            <div>
          ...
          {phase === "defense" && (
            <div className="max-w-2xl mx-auto space-y-2">
              ...
              {defenseCandidates.includes(userName) ? (
                defenseStatementCount >= 3 ? (
                  <div className="text-sm" style={{ color: C.textMuted }}>弁明は3回まで話しました。決選投票へ進んでください。</div>
                ) : (
                <>
                  <div className="text-sm font-bold" style={{ color: C.accent }}>あなたも決選投票の対象です。反論・弁明を書いてください。(残り{3 - defenseStatementCount}回)</div>
                  <div className="flex gap-2 items-end">
                    <textarea
                      ref={inputRef}
                      className="flex-1 rounded-lg px-3 py-2 outline-none border resize-none overflow-y-auto"
              ...
              <button onClick={() => enterVoteFinalPhase()} disabled={busy} className="w-full py-2 rounded-lg text-sm" style={{ background: "#EDE0D8", color: C.userLabel }}>決選投票へ進む</button>
            </div>
          )}
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-1.5 pb-2 border-b" style={{ borderColor: C.borderStrong }}>
                    <div className="text-xs font-bold" style={{ color: C.gold }}>🌙 仲間との密談(夜のみ・残り{Math.max(0, 3 - allyChatCount)}回)</div>
                    {allyChatCount >= 3 ? (
                      <div className="text-xs" style={{ color: C.textFaint }}>今夜の密談は3回話したので終わりです。</div>
                    ) : (
                    <div className="flex gap-2 items-end">
                      <textarea
